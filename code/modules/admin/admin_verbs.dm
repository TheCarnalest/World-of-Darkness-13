...
/client/proc/admin_ghost()
	...
	if(isobserver(mob))
		...
		if(ghost.aghosted)
			...
			ghost.movement_type = FLYING | PHASING | GROUND //[Lucifernix] - Uncommenting this makes aghosts unable to
		...
		/*if(!ghost.aghosted)
			...
			ghost.sight = 0
			ghost.movement_type = FLYING | GROUND | PHASING// [ChillRaccoon] - makes us available to go through dens objects
			*/
