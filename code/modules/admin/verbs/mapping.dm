...
/client/proc/create_mapping_job_icons()
	...
	for(var/job in subtypesof(/datum/job))
		...
		switch(JB.title)
			if("AI")
				ultimate.Insert(icon('icons/mob/ai.dmi', "ai", SOUTH, 1), "AI")
			if("Cyborg")
				ultimate.Insert(icon('icons/mob/robots.dmi', "robot", SOUTH, 1), "Cyborg")
			else
				...
				ultimate.Insert(I, JB.title)
	qdel(D)
	//Also add the x
	for(var/x_number in 1 to 4)
		ultimate.Insert(icon('icons/hud/screen_gen.dmi', "x[x_number == 1 ? "" : x_number]"), "x[x_number == 1 ? "" : x_number]")
	fcopy(ultimate, "icons/mob/landmarks.dmi")
