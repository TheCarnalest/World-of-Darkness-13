...
/mob/living/carbon/human/Topic(href, href_list)
	...
	if(href_list["pockets"] && (usr.canUseTopic(src, BE_CLOSE, NO_DEXTERITY) || (L.enhanced_strip && (range(usr, src) <= 6)))) //TODO: Make it match (or intergrate it into) strippanel so you get 'item cannot fit here' warnings if mob_can_equip fails
		if(isnpc(src))
