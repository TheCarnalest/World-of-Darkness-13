...
/obj/item/reagent_containers/vampire/methpack
	name = "a drinkable blood pack (full)"
...
/obj/item/reagent_containers/vampire/methpack/attack(mob/living/M, mob/living/user)
	. = ..()
	if(!iskindred(M))
		if(!vitae)
			return
	...
	if(do_mob(user, src, 3 SECONDS))
		var/obj/item/reagent_containers/vampire/methpack/H
		feeding = FALSE
		empty = TRUE
		icon_state = "blood0"
		...
		H.reagents.trans_to(src, min(15, H.reagents.total_volume), methods = VAMPIRE)
