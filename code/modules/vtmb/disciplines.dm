...
/datum/discipline/obfuscate/activate(mob/living/target, mob/living/carbon/human/owner)
	. = ..()
	...
	owner.alpha = 10
	owner.obfuscate_level = level_casting
	spawn((delay*level_casting)+owner.discipline_time_plus)
		if(owner)
			if(owner.alpha != 255)
				owner.playsound_local(owner.loc, 'code/modules/wod13/sounds/obfuscate_deactivate.ogg', 50, FALSE)
				owner.alpha = 255
