...
/datum/action/discipline/handle_click(mob/source, atom/target)
	SIGNAL_HANDLER
...
/datum/action/discipline/IsAvailable()
	. = ..()
	if (!discipline?.current_power.can_activate_untargeted())
		. = FALSE
	return .
...
/datum/action/discipline/Trigger()
	...
	if(power.target_type == NONE)
		power.try_activate()
	//ranged targeted activation
	else if (power.range > 0)
		begin_targeting()
	. = ..()
...
/datum/action/discipline/proc/switch_level()
	...
	if (discipline.equipped_powers[discipline.level_casting + 1])
